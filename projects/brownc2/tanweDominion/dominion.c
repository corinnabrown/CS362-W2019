...
void adventurerCard(struct gameState *state)
{
	...
	while(i - 1 >= 0){
		state->discard[currentPlayer][state->discardCount[currentPlayer]++] = temphand[i - 1]; // discard all cards in play that have been drawn
		i = i - 1;
	}
}
...
void feastCard(struct gameState *state, int choice)
{
	...
	j = 1;//Condition to loop on
	while( j == 1) {//Buy one card
